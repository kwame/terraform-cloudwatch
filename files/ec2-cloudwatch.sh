...
yum -y install git screen httpd tmux awslogs perl-Switch perl-DateTime perl-Sys-Syslog perl-LWP-Protocol-https irssi awesome
...
echo "*/1 * * * * ec2-user /opt/aws-scripts-mon/mon-put-instance-data.pl --swap-used --mem-util --swap-util --mem-used --disk-space-util --disk-space-used --from-cron" >> /etc/crontab
